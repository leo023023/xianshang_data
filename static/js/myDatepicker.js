...
;(function ($) {
    ...
    $.fn.myDatePicker = function (inOptions) {
        ...
        if ($.fn.swiperAxis == undefined) {
            ;(function ($) {
                $.fn.swiperAxis = function (inOptions) {
                    ...
                    let containerWidth;
                    let contentWidth;
                    let isX = options['direction'] == 'horizon';
                            ...
                            if (distance > correctMax || distance < correctMin) {
                                ...
                                func = function () {
                                    move(correctDistance, {
                                        useRaf: true, frame: 30, func: function () {
                                            isMoving = false
                                        }
                                    })
                                }
                    ...
                    function move(x, funOptions, isTrusted) {
                        ...
                        if (!moveOptions['useRaf']) {
                            ...
                            rafMove(x, [_object], isX ? 'margin-left' : 'margin-top', moveOptions['frame'], {
                                func: moveOptions['func'],
                                isTrusted: isTrusted
                            })
                        }
                    }
        ...
        $(this).each(function () {
            ...
            $("#biaodan__xb_nobirth").click(function () {
                $("#cssj_js").text("预产期:");
                // parent.location.reload();
                val_jsjs =3;
            })
            $("#biaodan__xb_nv").click(function () {
                $("#cssj_js").text("出生时间:");
                val_jsjs =6;
            })
            $("#biaodan__xb_nan").click(function () {
                $("#cssj_js").text("出生时间:");
                val_jsjs =6;
            })

            let options = {
                view: val_jsjs,//0 1 2 3 4 5 6 7 表示 年 月 日 周 季 时 分 秒
            ...
            let value = _this.val() == '' ? (new Date()).Format("yyyy-mm-dd hh:ii:ss") : _this.val(),
                valueToDate = value.toDate(format);
            let year = valueToDate.getFullYear(), month = (valueToDate.getMonth() + 1), day = valueToDate.getDate(),
                hour = valueToDate.getHours(), minute = valueToDate.getMinutes(), second = valueToDate.getSeconds(),
                maxYear, maxMonth, maxDay, maxHour, maxMinute, maxSecond, minYear, minMonth, minDay, minHour, minMinute,
                minSecond;
            //doms
